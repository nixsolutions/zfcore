...
class Model_Users_ManagerTest extends ControllerTestCase
{
    ...
    public function testSetPassword()
    {
        $user = $this->_userManager
                     ->forgetPassword($this->_fixture['admin']['email']);
        $result = $this->_userManager
                       ->setPassword($user->hashCode, 654321);
        $this->assertTrue($result);

        $result = $this->_userManager
                       ->setPassword(1010101010, 654321);
        $this->assertFalse($result);
    }

    /**
     * Clear user hash test
     */
    public function testClearHash()
    {
        $user = $this->_userManager
                     ->forgetPassword($this->_fixture['admin']['email']);
        $result = $this->_userManager
                       ->clearHash($user->hashCode);
        $this->assertTrue($result);

        $result = $this->_userManager
                       ->clearHash(1010101010);
        $this->assertFalse($result);
    }
