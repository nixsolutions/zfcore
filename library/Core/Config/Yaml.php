...
class Core_Config_Yaml extends Zend_Config
{
     ...
     * @var bool
     */
    protected static $_ignoreConstants = true;

    /**
     * Whether or not to ignore predefined constants in parsed YAML
     * @var bool
     */
    protected static $_ignoreDefinitions = false;

    /**
     * Predefined constant names in parsed YAML
     * <code>array(
     * 'APPLICATION_ENV',
     * 'APPLICATION_PATH'
     * )
     ...
     * @return void
     */
    public static function setIgnoreDefinitions($flag)
    {
        self::$_ignoreDefinitions = (bool) $flag;
    }
     ...
     * @return bool
     */
    public static function ignoreDefinitions()
    {
        return self::$_ignoreDefinitions;
    }
     ...
     * @return void
     */
    public static function setDefinitions($data)
    {
        self::$_definitions = (array) $data;
    }
            ...
            foreach ($options as $key => $value) {
                switch (strtolower($key)) {
                    ...
                    case 'ignoreconstants':
                        ...
                        break;
                    case 'ignore_definitions':
                    case 'ignoredefinitions':
        ...
        while (list($n, $line) = each($lines)) {
            ...
            if (preg_match("/([a-zA-Z0-9-_]+):\s*(.*)/", $line, $m)) {
                ...
                if ($m[2]) {
                    ...
                    } elseif (!self::$_ignoreDefinitions) {
                        // test for predefined constants
                        $value = self::_replaceDefinitions($value);
                    }
                } else {
     ...
     * @return string
     */
    protected static function _replaceDefinitions($value)
    {
        foreach (self::$_definitions as $constant) {
            if (strstr($value, $constant)) {
                $value = str_replace($constant, constant($constant), $value);
            }
        }
