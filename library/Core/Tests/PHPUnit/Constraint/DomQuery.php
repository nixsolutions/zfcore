<?php
/**
 * Core_Dom_Query-based PHPUnit Constraint
 *
 ...
 * @subpackage PHPUnit
 */
class Core_Tests_PHPUnit_Constraint_DomQuery
    extends PHPUnit_Framework_Constraint
{
     ...
     * @return void
     */
    public function __construct($path, $assertType = null, $content = null)
    {
        $this->_path = $path;
     ...
     *
     * @param  bool $flag
     * @return Core_Tests_PHPUnit_Constraint_DomQuery
     */
    ...
    public function evaluate($other, $description = '', $returnResult = false)
    {
        ...
        switch ($this->_assertType) {
            case self::ASSERT_CONTENT_CONTAINS:
                return ($this->_negate)
                    ? $this->_notMatchContent($result, $this->_content)
                    : $this->_matchContent($result, $this->_content);
            case self::ASSERT_CONTENT_REGEX:
                return ($this->_negate)
                    ? $this->_notRegexContent($result, $this->_content)
                    : $this->_regexContent($result, $this->_content);
            case self::ASSERT_CONTENT_COUNT:
            case self::ASSERT_CONTENT_COUNT_MIN:
            case self::ASSERT_CONTENT_COUNT_MAX:
                return $this->_countContent(
                    $result,
                    $this->_content,
                    $this->_assertType
                );
        ...
        PHPUnit_Framework_ComparisonFailure $comparisonFailure = NULL)
    {
        ...
        switch ($this->_assertType) {
            case self::ASSERT_CONTENT_CONTAINS:
                $failure = 'Failed asserting node '
                    . 'denoted by %s CONTAINS content "%s"';
                if ($this->_negate) {
                    $failure = 'Failed asserting node '
                        . 'DENOTED BY %s DOES NOT CONTAIN content "%s"';
                }
                $failure = sprintf($failure, $other, $this->_content);
                break;
            case self::ASSERT_CONTENT_REGEX:
                $failure = 'Failed asserting node denoted by '
                    . '%s CONTAINS content MATCHING "%s"';
                if ($this->_negate) {
                    $failure = 'Failed asserting node DENOTED '
                        . 'BY %s DOES NOT CONTAIN content MATCHING "%s"';
                }
                $failure = sprintf($failure, $other, $this->_content);
                break;
            case self::ASSERT_CONTENT_COUNT:
                $failure = 'Failed asserting node DENOTED BY %s OCCURS '
                    . 'EXACTLY %d times';
                if ($this->_negate) {
                    $failure = 'Failed asserting node DENOTED BY %s DOES NOT '
                        . 'OCCUR EXACTLY %d times';
                }
                $failure = sprintf($failure, $other, $this->_content);
                break;
            case self::ASSERT_CONTENT_COUNT_MIN:
                $failure = 'Failed asserting node DENOTED BY %s OCCURS AT '
                    . 'LEAST %d times';
                $failure = sprintf($failure, $other, $this->_content);
                break;
            case self::ASSERT_CONTENT_COUNT_MAX:
            ...
            default:
                ...
                if ($this->_negate) {
                    $failure = 'Failed asserting node DENOTED BY %s '
                        . 'DOES NOT EXIST';
                }
