 ...
 * @subpackage PHPUnit
 */
abstract class Core_Tests_PHPUnit_ControllerTestCase
    extends PHPUnit_Framework_TestCase
{
    ...
    public function __set($name, $value)
    {
        if (in_array($name, array('request', 'response', 'frontController'))) {
            ...
            throw new Zend_Exception(
                sprintf('Setting %s object manually is not allowed', $name)
            );
        }
        $this->$name = $value;
    }
     ...
     *
     * If {@link $bootstrap} is a callback, executes it; if it is a file,
     * it include's it.
     * When done, sets the test case request and response objects into the
     * front controller.
     *
     * @return void
     */
     ...
     * Useful for test cases that need to test multiple trips to the server.
     *
     * @return Core_Tests_PHPUnit_ControllerTestCase
     */
    ...
    public function assertQuery($path, $message = '')
    {
        ...
        $constraint = new Core_Tests_PHPUnit_Constraint_DomQuery(
            $path,
            __FUNCTION__
        );
     ...
     *
     * @param  string $path CSS selector path
     * @param  string $match content that should NOT be contained in
     * matched nodes
     * @param  string $message
     * @return void
     */
     ...
     *
     * @param  string $path CSS selector path
     * @param  string $pattern pattern that should NOT be contained in matched
     * nodes
     * @param  string $message
     * @return void
     */
     ...
     *
     * @param  string $path XPath path
     * @param  string $match content that should NOT be contained in matched
     * nodes
     * @param  string $message
     * @return void
     */
    ...
    /**
     * Assert against XPath selection; should contain at least this number of
     * nodes
     *
     * @param  string $path XPath path
     * @param  string $count Minimum number of nodes that should match
    ...
    {
        $this->_incrementAssertionCount();
        require_once 'Core/Tests/PHPUnit/Constraint/Redirect.php';
        $constraint = new Core_Tests_PHPUnit_Constraint_Redirect(__FUNCTION__);
        $response   = $this->response;
        if (!$constraint->evaluate($response)) {
    ...
    public function assertRedirectTo($url, $message = '')
    {
        $this->_incrementAssertionCount();
        require_once 'Core/Tests/PHPUnit/Constraint/Redirect.php';
        $constraint = new Core_Tests_PHPUnit_Constraint_Redirect(
            __FUNCTION__,
            $url
        );
        $response   = $this->response;
    ...
    {
        $this->_incrementAssertionCount();
        require_once 'Core/Tests/PHPUnit/Constraint/ResponseHeader.php';
        $constraint = new Core_Tests_PHPUnit_Constraint_ResponseHeader(
            __FUNCTION__,
            $code
        );
        $response   = $this->response;
    ...
    /**
     * Assert response header does not exist
     * and/or does not contain the given string
     *
     * @param  string $header
     * @param  string $match
     * @param  string $message
    ...
    /**
     * Assert response header does not exist
     * and/or does not match the given regex
     *
     * @param  string $header
     * @param  string $pattern
     * @param  string $message
    ...
    public function assertNotModule($module, $message = '')
    {
        ...
        if ($module == $this->request->getModuleName()) {
            $msg = sprintf(
                'Failed asserting last module used was NOT "%s"',
                $module
            );
            if (!empty($message)) {
                $msg = $message . "\n" . $msg;
    ...
    public function assertAction($action, $message = '')
    {
        ...
        if ($action != $this->request->getActionName()) {
            $msg = sprintf(
                'Failed asserting last action used <"%s"> was "%s"',
    ...
    public function assertNotAction($action, $message = '')
    {
        ...
        if ($action == $this->request->getActionName()) {
            $msg = sprintf(
                'Failed asserting last action used <"%s"> was NOT "%s"',
                $this->request->getActionName(),
                $action
            );
            if (!empty($message)) {
     ...
     */
    public function url(
        $urlOptions = array(), $name = null, $reset = false, $encode = true)
    {
        $frontController = $this->getFrontController();
        $router = $frontController->getRouter();
        if (!$router instanceof Zend_Controller_Router_Rewrite) {
            throw new Exception(
                'This url helper utility function only works when the router '
                    . 'is of type Zend_Controller_Router_Rewrite'
            );
        }
    ...
    public function urlizeOptions(
        $urlOptions, $actionControllerModuleOnly = true)
    {
