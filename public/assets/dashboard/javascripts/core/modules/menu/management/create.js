;(function(core, $, window, undefined) {
    ...
    /** init namespace */
    var create = core.namespace('core.modules.menu.management.create');

    function getSelect(label, name) {
        return '<dt><label for="' + name + 's-list">' + label + '</label></dt>'
             + '<dd><select name="params[' + name + ']" class="ui-state-default ui-corner-all" id="' + name + 's-list">' + '</select></dd>';
    }
    ...
    create.init = function() {
        ...
        $('#newparam-button').live('click', function() {
            ...
            if (val) {
                var template = '<dt><label for="newparam' + val + '">' + val + ':</label></dt>'
                             + '<dd><input name="params[' + val + ']" class="ui-state-default ui-corner-all" id="' + val + '"/></dd>';
